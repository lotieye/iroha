...
std::unique_ptr<JavaContext> initialize(
  ...
  std::vector<std::string> java_args = {
      "-Djava.class.path=" + IrohaHome + "/" +
          config::IrohaConfigManager::getInstance().getJavaClassPath(
              "java_tests") +
          ":" + IrohaHome + "/" +
          config::IrohaConfigManager::getInstance().getJavaClassPathLocal(
              "smart_contract/java_tests"),
      "-Djava.library.path=" + IrohaHome + "/" +
          config::IrohaConfigManager::getInstance().getJavaLibraryPath("lib") +
          ":" + IrohaHome + "/" +
          config::IrohaConfigManager::getInstance().getJavaLibraryPathLocal(
              "build/lib"),
      "-Djava.security.policy=" + IrohaHome + "/" +
          config::IrohaConfigManager::getInstance().getJavaPolicyPath(
              "jvm/java.policy.txt"),
...
void execFunction(const std::unique_ptr<JavaContext>& context,
  ...
  jmethodID mid = context->env->GetStaticMethodID(
      context->jClass, functionName.c_str(), "(Ljava/util/HashMap;)V");
  if (mid == nullptr) {
...
void execFunction(const std::unique_ptr<JavaContext>& context,
  ...
  jmethodID mid = context->env->GetStaticMethodID(
      context->jClass, functionName.c_str(),
...
void execFunction(const std::unique_ptr<JavaContext>& context,
                  const std::string& functionName) {
  jmethodID mid = context->env->GetStaticMethodID(context->jClass,
                                                  functionName.c_str(), "()V");
  if (mid == nullptr) {
    std::cout << "could not get method : " << functionName << std::endl;
    return;
  }
